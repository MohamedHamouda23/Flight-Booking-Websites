...
```bash
/usr/local/mysql/bin/mysql -u Username -p -e "CREATE DATABASE IF NOT EXISTS \`Horizon_travel\`;" ; /usr/local/mysql/bin/mysql -u Username -p Horizon_travel < horizon_travel.sql
```
